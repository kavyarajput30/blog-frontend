...
function DashboardComp() {
  ...
  const fetchUsers = async () => {
    try {
      const res = await axios.get("/api/v1/user/get-users?limit=5");
      if (res.data.success) {
  ...
  const fetchPosts = async () => {
    try {
      const res = await axios.get("/api/v1/post/get-posts?limit=5");
      if (res.data.success) {
        setPosts(res.data.data.posts);
        setTotalPosts(res.data.data.totalPosts);
        setLastMonthPosts(res.data.data.lastMonthsPosts);
      }
    } catch (err) {
      console.log(err);
    }
